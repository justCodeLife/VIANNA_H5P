...
H5P.instances.push(instance);H5P.trigger(instance,'resize');$element.addClass('using-mouse');$element.on('mousedown keydown keyup',function(event){$element.toggleClass('using-mouse',event.type==='mousedown');});if(H5P.externalDispatcher){H5P.externalDispatcher.trigger('initialized');}});H5P.jQuery('iframe.h5p-iframe:not(.h5p-initialized)',target).each(function(){var contentId=H5P.jQuery(this).addClass('h5p-initialized').data('content-id');const contentData=H5PIntegration.contents['cid-'+contentId];const language=contentData&&contentData.metadata&&contentData.metadata.defaultLanguage?contentData.metadata.defaultLanguage:'en';this.contentDocument.open();this.contentDocument.write('<!doctype html><html class="h5p-iframe" lang="'+language+'"><head>'+H5P.getHeadTags(contentId)+'</head><body><div class="h5p-content" data-content-id="'+contentId+'"/></body></html>');this.contentDocument.close();});};H5P.getHeadTags=function(contentId){var createStyleTags=function(styles){var tags='';for(var i=0;i<styles.length;i++){tags+='<link rel="stylesheet" href="'+styles[i]+'">';}
...
if($attachTo!==undefined){$attachTo.toggleClass('h5p-standalone',standalone);if(library.params.language==null||library.params.language==undefined){$attachTo.toggleClass(' h5p-dir-'+window.parent.document.dir);}else{if(library.params.language=='fa'||library.params.language=='ar'){$attachTo.toggleClass('lang-'+library.params.language+' h5p-dir-rtl');}}
instance.attach($attachTo);H5P.trigger(instance,'domChanged',{'$target':$attachTo,'library':machineName,'key':'newLibrary'},{'bubbles':true,'external':true});if(skipResize===undefined||!skipResize){H5P.trigger(instance,'resize');}}
